...
const Project = () => {
  return (
    <div className=' '>
      <div className='flex justify-center gap-[2.5rem] IPad:gap-[2rem] side-phone:gap-2'> 

      <div className='text-white flex justify-center items-center  relative  py-[5.7rem] IPad:pl-[4rem] side-phone:py-[11rem] side-phone:pl-[0]'>
 ...
 <button className='text-white bg-[#067EF6] px-4 py-3.5 text-[0.9rem] rounded-[2rem] IPad:text-[0.7rem] side-phone:text-[0.6rem] transition duration-300 ease-in-out transform hover:scale-105'>Add New Project</button>
      ...
      {projectData.map((project) => (
        <div key={project.id} className='flex justify-center flex-col'>
          <div className='bg-white w-[18rem]  h-[10rem] rounded-lg'></div>
          <div className='bg-black w-[19rem] h-[9rem] mt-[-1rem] overflow-hidden rounded-b-[1rem] 

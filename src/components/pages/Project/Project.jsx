...
const Project = ({ onProjectsChange }) => {
  ...
  const fetchProjects = async () => {
    ...
    if (storedProjects) {
      ...
      onProjectsChange(projectsData.length);
  ...
  const fetchFreshData = async () => {
    try {
      ...
      console.log('Fetched fresh projects:', response);
  ...
  const handleDelete = async (project_id) => {
    try {
      console.log(`Attempting to delete project with project_id: ${project_id}`);
      await deleteProject(project_id);
      console.log(`Successfully deleted project with project_id: ${project_id}`);
      // After successful deletion, update both state and localStorage
      const updatedProjects = projects.filter(project => project.id !== project_id);
      setProjects(updatedProjects);
      localStorage.setItem('projects', JSON.stringify(updatedProjects));
      localStorage.setItem('lastFetchTime', new Date().getTime().toString());
      onProjectsChange(updatedProjects.length);
    } catch (error) {
  ...
  const currentProjects = Array.isArray(projects) ? projects.slice(indexOfFirstPost, indexOfLastPost) : [];
  console.log('Current projects:', currentProjects); // Debugging log
            ...
            return (
              <div key={project.id} className='flex justify-center flex-col'>
                <div className='image_urls bg-white side-phone:w-[16.5rem] side-phone:h-[9.2rem] w-[18rem]  h-[10rem] rounded-lg'>
                  <img src={project.image_urls} alt={project.name} className='w-full h-full object-cover rounded-lg' />
                </div>
